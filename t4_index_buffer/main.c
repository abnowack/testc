...
void construct_Shape(struct Shape *t)
{
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * t->n_indices, t->indices, GL_STATIC_DRAW);
...
{
    glBindVertexArray(t->VAO);
    glDrawElements(GL_TRIANGLES, t->n_indices, GL_UNSIGNED_INT, 0);
}
...
int main()
{
    ...
    for (unsigned int i = 0; i < N_SHAPES; i++)
    {
        ...
        // else
        // {
            triangles[i] = random_Rectangle(0.3f);
        // }
